...
const DiscountAnalysisInputSchema = z.object({
  ...
  cogs: z.number().describe('The Cost of Goods Sold for the product.'),
  discountPercentage: z.number().describe('The proposed discount percentage (e.g., 20 for 20%).'),
  profitAfterDiscount: z.number().describe('The calculated profit per sale *after* the discount is applied.'),
...
const DiscountAnalysisOutputSchema = z.object({
  ...
  analysis: z.string().describe('A detailed analysis of the discount\'s impact, written in HTML format. Explain the trade-offs. Use <p>, <ul>, <li>, and <strong> tags.'),
...
You are analyzing the following proposed discount for a D2C brand:
