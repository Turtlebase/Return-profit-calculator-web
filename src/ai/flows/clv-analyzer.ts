...
const ClvAnalysisInputSchema = z.object({
  aov: z.number().describe('Average Order Value (AOV) in Indian Rupees (₹).'),
  ...
  clv: z.number().describe('The calculated Customer Lifetime Value in Indian Rupees (₹).'),
});
...
const ClvAnalysisOutputSchema = z.object({
  ...
  analysis: z.string().describe('A detailed analysis of the CLV, written in HTML format. Explain what the CLV means in practical terms. Use <p>, <ul>, <li>, and <strong> tags. All monetary values must be in Indian Rupees (₹).'),
...
You are analyzing the following CLV data for a D2C brand. All monetary values are in Indian Rupees (₹).
